...
> **ScholarRAG 论文助手开发版**
>
> 本仓库正在将通用 RAG 框架改造成团队论文检索助手：目标支持约 20 名用户共享约 500 篇论文，并根据模糊记忆找回目标论文。当前已完成 Windows 运行兼容性修复、论文级 Paper Profile、版本去重、加权 BM25、本地中文 Dense 检索、论文向量与正文分块的 Chroma 持久化、未知论文阈值拒答、RRF 混合排序、两级“论文路由 → 页级正文检索”、可选的本地 ONNX Cross-Encoder 重排、页码与证据级离线评测、`find_paper` MCP 工具、本地论文目录盘点、带原文证据的候选档案生成和人工确认后的受控增量入库，并建立首批 10 篇独立论文（11 个本地 PDF）、30 条已知论文种子问题、9 条开发用未知论文问题和 10 条正文证据开发题。论文 PDF、文件哈希和本地盘点报告不进入公开仓库。
>
