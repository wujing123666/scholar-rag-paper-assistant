...
1. 先关闭论文档案、论文目录、已有测试集和检索界面。
2. 一次性写完全部 9～18 条自然语言描述，期间不核对任何论文信息。
3. 所有描述保存后，再统一确认目标 PDF 或标题，不能反向补关键词。
...
### 2026-09-25：严格化盲测填写顺序

**发现的问题**

如果每写一条描述就立刻打开论文目录核对目标文件，文件名和相邻论文可能提醒后续问题的关键词，降低盲测独立性。

**修改**

将采集顺序改为“两阶段”：第一阶段在完全不查看论文信息的情况下，一次性写完并保存全部模糊描述；第二阶段才统一补充目标论文、难度和线索类型。目标答案只能补在独立字段中，不能反向修改描述。

**面试表达**

> 盲测问题采用两阶段采集。先一次性冻结所有自然语言查询，再统一标注标准答案，避免标注过程给后续查询泄漏论文标题或文件名。

**下一步**

等待用户提交第一版冻结描述，随后只做答案映射和歧义检查，不做检索导向的改写。
