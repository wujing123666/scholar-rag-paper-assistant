...
1. 不查看 `paper_catalog.csv`、已有的 `eval_queries.jsonl`、论文摘要和检索结果。
2. 先填写“模糊描述”。描述写完后不要再润色关键词。
3. 再打开本地论文目录，只补充“目标论文”，不要修改模糊描述。
4. 最后填写难度、线索类型和个人备注。
5. 在“是否冻结”处写“是”。冻结后不根据系统返回结果修改问题。

目标论文可以填写你认识的 PDF 文件名、论文标题或简称，不要求你知道 `paper_id`。后续由程序维护者映射成标准答案。
